function [L, P, gammaM, Nf, eTot] = csspod(X, a0, nt0, dt, opts)
...
% Compute what frequencies are needed for each gamma
ellM = zeros(nf_total, numGam);
...
for iGam = 1:numGam
    ...
    % Compute all the frequencies we need for CS-SPOD
    for indx_f = 1:nf_total
        ...
        if k <= floor(c/2)+1
            if kf >= 0
                ellM(indx_f, iGam) = k + kf*c;
            elseif kf < 0
                ellM(indx_f, iGam) = k + kf*c + nDFT;
            end
    ...
    if max(abs(diff(f(ellM(:, iGam))) - a0)) > 1e-10
...
dataMF = zeros(nx, nf_total, nBlks, numGam, class(X)); dataMF = complex(dataMF, 0);
...
for iGam = 1:numGam
    ...
    Qtilde = reshape(Qtilde, [nx*nf_total, nBlks]);
