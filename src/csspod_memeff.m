function [L, P, gammaM, Nf, eTot] = csspod_memeff(X, a0, nt0, dt, opts)
...
%   from [1]. CS-SPOD is an extension of SPOD [4] to flows with peirodic
%   statistics. It is a multidimensional version of [3] using the 
%   method-of-snapshot technique for computational feasibility. 
%   CS-SPOD has been applied to a turbulent jet in [1, 2]. 
% 
%   In this version of the algorithm, a less computationally efficient order of
%   operations is performed which can drastically reduce the memory usage.
%   This algorithm is less efficient than csspod() since nf_total FFT's of
%   the original data is performed instead of a single FFT.
...
if ~quiet
    disp('**********************************************************************');
    disp('Starting CS-SPOD computation using f-shifting (efficient) algorithm');
    disp('employing a less efficient order of operations to reduce memory usage.');
    disp('**********************************************************************');
end
...
if ~quiet
    ...
    disp(['        - Window length: ', num2str(length(Window)), ' (c = ', num2str(length(Window)/nt0), ', \Delta gamma = ', num2str(delf, 4), ')']);
...
if abs(a0*nt0*dt - 1) > 1e-10; error('a0*nt0*dt =/= 1'); end
...
% Numner of gamma values in total
...
for iGam = 1:numGam
    ...
    % Get the current gammaIndx
    k = gammaM_k(iGam);
    
    % Init index values
    ell = zeros(nf_total, 1);
    
    % Loop over all frequency shifts of the data
    for indx_f = 1:nf_total
        
        % Current frequency offset
        kf = nfM(indx_f);
        
        % Compute the index of the different frequency components (equation 3.26)
        if k <= floor(c/2)+1
            if kf >= 0
                ell(indx_f) = k + kf*c;
            elseif kf < 0
                ell(indx_f) = k + kf*c + nDFT;
            end
        elseif k > nDFT-ceil(c/2)+1 && k <= nDFT
            if kf > 0
                ell(indx_f) = k + kf*c - nDFT;
            elseif kf <= 0
                ell(indx_f) = k + kf*c;
    ...
    if max(abs(diff(f(ell)) - a0)) > 1e-10
        error('Different between the frequency components is not as expected');
    end
    
    
    %% Compute the frequency data matrix
    if ~quiet; disp('            - Computing frequency data matrix'); end
    
    % Init the data matrix with the same type as the orignal matrix
    dataMF = zeros(nf_total, nx, nBlks, class(X)); dataMF = complex(dataMF, 0);
    
    % Save the data matrix offset
    offsetM = zeros(1, nBlks);
    ...
    for iBlk = 1:nBlks
        ...
        % Save the off set to the offset matrix
        offsetM(iBlk) = offset;
        ...
        curDatF = fft(curDat.*Window, [], 1);
        dataMF(:, :, iBlk) = curDatF(ell, :);
        
    end
    
    %% Compute CS-SPOD using the method of snapshot technique
    if ~quiet; disp('            - Computing eigendecomposition'); end
    
    % Generate the concatenated frequency-data matrix (equation 3.14b)
    % using equation 3.23b to efficient compute it
    Qtilde = zeros(nx*nf_total, nBlks, class(X)); Qtilde = complex(Qtilde, 0);
    
    % Loop over each entry in the concatenated frequency-data matrix
    for indx_f = 1:nf_total
        
        % Current frequency offset
        kf = nfM(indx_f);
        
        % Start and end index of the matrix
        startIndx = (indx_f-1)*nx +1;
        endIndx = indx_f*nx;
        
        % Extract the data and do the phase-shifting required by this
        % efficient method (equation 3.23b)
        cDatTemp = squeeze(dataMF(indx_f, :, :)).*exp(-1i*2*pi*kf*a0*dt*offsetM);
        
        % Insert into the matrix
        Qtilde(startIndx:endIndx, :) = cDatTemp;
